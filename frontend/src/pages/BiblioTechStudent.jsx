...
/* ═══════════════════════════════════════════════════════════════
   ...
   Teal accent · Space Grotesk + Plus Jakarta Sans
   Borrow → sends request to Admin panel for approval
═══════════════════════════════════════════════════════════════ */

const T = {
  bg:"#07090f", surface:"#111420", card:"#161a28",
...
const BOOKS=[
  ...
  {id:"B007",title:"Business Law",author:"Henry R. Cheeseman",dept:"Law",rating:4.4,borrows:55,status:"available",cover:["#1c1917","#292524","#44403c"],year:2019,isbn:"978-0135085929",pages:912,desc:"Comprehensive coverage of the legal environment of business and commercial law.",isNew:false},
...
const CSS=`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .card:hover{transform:translateY(-7px) scale(1.025);}
...
// ── BOOK DETAIL with Borrow Request ──────────────────────────────
      ...
      <div style={{display:"grid",gridTemplateColumns:"310px 1fr",gap:36}}>
        <div>
          <div style={{borderRadius:18,overflow:"hidden",border:`2px solid ${dc}44`,
            boxShadow:`0 28px 70px ${book.cover[1]}40`,
          ...
          <div style={{background:T.surface,border:`1px solid ${T.border}`,
            borderRadius:16,padding:"20px",marginTop:18}}>
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <span style={{background:dc+"22",color:dc,border:`1px solid ${dc}44`,
              ...
              fontFamily:"'Space Grotesk',sans-serif"}}>{book.dept}</span>
            <span style={{fontSize:13,color:"#f59e0b",display:"flex",alignItems:"center",gap:5,
              fontFamily:"'Space Grotesk',sans-serif",fontWeight:600}}>
              <Ic p={P.star} s={13} fill color="#f59e0b"/>{book.rating}/5
            </span>
            <span style={{fontSize:13,color:T.sub,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{book.borrows} borrows</span>
          </div>
          ...
          <p style={{fontSize:15,color:T.sub,lineHeight:1.8,marginBottom:30,
            ...
            borderRadius:14,padding:"20px 22px",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{book.desc}</p>

          {/* Borrow Request Status */}
          {requested?(
            <div style={{background:"#f59e0b12",border:"1px solid #f59e0b40",
              borderRadius:16,padding:"24px 28px",
              display:"flex",alignItems:"center",gap:16,animation:"scaleIn 0.3s ease"}}>
              ...
              <div>
                <p style={{fontSize:16,fontWeight:700,color:T.text,marginBottom:5,
                  fontFamily:"'Space Grotesk',sans-serif"}}>Request Sent — Awaiting Approval</p>
                <p style={{fontSize:13,color:T.sub,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
                  The librarian will review your request. You will be notified when approved.
                </p>
              </div>
            </div>
          ):(
            <div style={{display:"flex",gap:14}}>
              <button onClick={handleBorrow}
                disabled={book.status!=="available"||sending} className="btn"
                style={{flex:2,background:book.status==="available"
                  ?`linear-gradient(135deg,${T.prime},${T.primeD})`:T.dim,
                ...
                {sending
                  ...
                  :book.status==="available"
                    ?<><Ic p={P.bookOpen} s={17}/> Request to Borrow</>
                    :sc.label}
              </button>
          ...
          {/* Info note */}
          {book.status==="available"&&!requested&&(
            <p style={{fontSize:12,color:T.muted,marginTop:14,
              fontFamily:"'Plus Jakarta Sans',sans-serif",
              display:"flex",alignItems:"center",gap:6}}>
              <Ic p={P.pending} s={13}/>
              Requests are reviewed by the librarian before approval.
            </p>
          )}
        </div>
      </div>
        ...
        <div>
          <SHead accent={T.cyan} label="Trending Now" sub="Most borrowed this week"
            extra={<span onClick={()=>onNavigate("explore")}
              style={{fontSize:13,color:T.prime,fontWeight:600,cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif"}}>Browse all →</span>}
          />
          ...
          <div style={{flex:1,position:"relative"}}>
            ...
            <input className="inp" value={q} onChange={e=>setQ(e.target.value)}
              ...
              style={{width:"100%",background:T.card,border:`1px solid ${T.border}`,
                ...
                color:T.text,fontSize:14,fontFamily:"'Plus Jakarta Sans',sans-serif"}}/>
          </div>
...
function MyLibraryPage({user}){
  ...
  return(
    <div style={{padding:"36px",fontFamily:"'Plus Jakarta Sans','Segoe UI',sans-serif"}}>
      <div style={{background:`linear-gradient(135deg,${T.surface},${T.card})`,
        border:`1px solid ${T.border}`,borderRadius:20,padding:"30px 36px",
        marginBottom:30,display:"flex",alignItems:"center",gap:26,
        ...
        <div style={{width:72,height:72,borderRadius:20,flexShrink:0,
          background:`linear-gradient(135deg,#1d4ed8,${T.cyan}50)`,
        ...
        <div style={{flex:1}}>
          ...
          <p style={{fontSize:14,color:T.sub}}>
            Card ID: <strong style={{color:T.text,fontFamily:"'Space Grotesk',sans-serif"}}>{user.libId}</strong> · {user.dept}
          </p>
        </div>
        ...
        {[["loans","Current Loans"],["requests","My Requests"],["history","History"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} className="btn"
            style={{background:tab===id?`linear-gradient(135deg,${T.prime},${T.primeD})`:"transparent",
              ...
              boxShadow:tab===id?`0 4px 14px ${T.primeG}`:"none",
              position:"relative"}}>
            {label}
          ...
          {MY_LOANS.map((loan,i)=>{
            ...
            return(
              <div key={i} style={{background:T.surface,border:`1px solid ${over?"#ef444440":T.border}`,
                ...
                <div style={{flex:1}}>
                  <p style={{fontSize:16,fontWeight:700,color:T.text,marginBottom:5,
                    fontFamily:"'Space Grotesk',sans-serif"}}>{loan.title}</p>
                ...
                <div style={{textAlign:"center",background:over?"#ef444418":soon?"#f59e0b18":"#22c55e18",
                  ...
                  <p style={{fontSize:11,fontWeight:600,color:sc,textTransform:"uppercase",
                    letterSpacing:"0.07em",marginTop:3,fontFamily:"'Space Grotesk',sans-serif"}}>
        ...
        <div>
          {myRequests.length===0?(
            <div style={{textAlign:"center",padding:"60px 20px"}}>
              <div style={{color:T.muted,display:"flex",justifyContent:"center",marginBottom:16}}>
                <Ic p={P.bookOpen} s={52}/></div>
              <p style={{fontSize:17,fontWeight:700,color:T.text,marginBottom:8,
                fontFamily:"'Space Grotesk',sans-serif"}}>No requests yet</p>
              <p style={{fontSize:14,color:T.sub,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
                  ...
                  <div key={r.id} style={{background:T.surface,border:`1px solid ${T.border}`,
                    ...
                    <div style={{flex:1}}>
                      <p style={{fontSize:15,fontWeight:700,color:T.text,marginBottom:4,
                        fontFamily:"'Space Grotesk',sans-serif"}}>{r.bookTitle}</p>
                    ...
                    <div style={{background:sc.bg,border:`1px solid ${sc.border}`,
                      ...
                      <Ic p={isPending?P.pending:P.approved} s={14}/>
                      {sc.label}
                    </div>
                  </div>
                );
          ...
          {READ_HISTORY.map((b,i)=>(
            <div key={i} style={{background:T.surface,border:`1px solid ${T.border}`,
              ...
              <div style={{padding:"16px 18px"}}>
                <p style={{fontSize:14,fontWeight:700,color:T.text,marginBottom:5,
                  fontFamily:"'Space Grotesk',sans-serif",
      ...
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        <div style={{position:"relative"}}>
          <div style={{width:42,height:42,borderRadius:12,background:T.surface,
            ...
            display:"flex",alignItems:"center",justifyContent:"center",color:T.sub,cursor:"pointer"}}>
            <Ic p={P.bell} s={18}/></div>
          <span style={{position:"absolute",top:9,right:9,width:8,height:8,
            borderRadius:"50%",background:T.prime,
            border:`1.5px solid ${T.bg}`,animation:"pulse 2s infinite"}}/>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10,
          background:T.surface,border:`1px solid ${T.border}`,
          borderRadius:12,padding:"6px 16px 6px 8px",cursor:"pointer"}}>
