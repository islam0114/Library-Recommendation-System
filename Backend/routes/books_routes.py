...
PUT  /api/admin/books/bulk-delete <- Bulk delete multiple books
GET  /api/books                   <- Supports sorting parameters
══════════════════════════════════════════════════════════
"""
...
# ── Configure directory for saving local book cover images ──
...
def _book_to_dict(row: dict, base_url: str = "") -> dict:
    """
    Helper function to format a database row into a standardized dictionary response.
    Handles both external image URLs and local uploaded covers.
    """
    ...
    return {
        ...
        "borrows":      row.get("borrows_count", 0),  # Track total borrows for analytics
...
def make_books_router(get_db, require_admin):
    ...
    async def get_stats(admin=Depends(require_admin), db=Depends(get_db)):
        """Fetch general statistics for the admin dashboard."""
        stats = {}
    ...
    async def update_book(book_id: int, body: BookUpdate, admin=Depends(require_admin), db=Depends(get_db)):
        """Update existing book details (Admin Only)."""
        ...
        # Reset local image if a new external URL is provided
        if body.image_url   is not None:
            fields.append("image_url=%s");   params.append(body.image_url.strip() or None)
            fields.append("image_local=%s"); params.append(None)
            
        # Dynamically calculate available copies when total copies are updated
    ...
    async def delete_book(book_id: int, admin=Depends(require_admin), db=Depends(get_db)):
        """Delete a book and its associated local cover image (Admin Only)."""
    ...
    async def bulk_delete_books(body: BulkDeleteBody, admin=Depends(require_admin), db=Depends(get_db)):
        """Delete multiple books and their associated images at once (Admin Only)."""
    ...
    async def upload_book_cover(book_id: int, file: UploadFile = File(...), admin=Depends(require_admin), db=Depends(get_db)):
        """Upload a local cover image for a specific book (Admin Only)."""
        await db.execute("SELECT image_local FROM books WHERE id=%s", (book_id,))
        row = await db.fetchone()
        ...
        # Generate unique filename to prevent cache conflicts
    ...
    async def get_book_cover(filename: str):
        """Serve locally uploaded book cover images."""
